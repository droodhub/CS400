...
public class SkipList<KeyType extends Comparable<KeyType>, ValueType> implements SortedMultiMap<KeyType, ValueType> {
     ...
     * The HeadNode does not store data itself (its key and value reference point to null), but
    ...
    protected SkipNode getInternal(KeyType key) {
        ...
        // the node with the key we are looking for or a key
    ...
    public KeyType getLargestKey() {
        ...
        int currentLevel = head.next.length - 1;
        while(currentLevel >= 0 && currentNode.getNext(currentLevel) != null) {
            currentNode = currentNode.getNext(currentLevel);
            if(currentNode.getNext(currentLevel) == null)
                currentLevel--;
        }
    ...
    public Iterator<ValueType> getRange(KeyType start, KeyType end) {
        if(start.compareTo(getSmallestKey()) < 0)
            start = getSmallestKey();
        KeyType finalStart = start;

        Iterator<ValueType> rangeIterator = new Iterator(){
            SkipNode currentNode = getInternalClosest(finalStart);
            public boolean hasNext(){
                if(currentNode == null)
                    return false;
                return (currentNode.key.compareTo(end) <= 0);
            }
            public ValueType next(){
                if(currentNode == head)
                    currentNode = currentNode.getNext(0);
                if(currentNode.key.compareTo(end) <= 0 && hasNext()) {
