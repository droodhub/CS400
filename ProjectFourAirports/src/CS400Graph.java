...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path copyPath, Edge extendBy) {
            ...
            this.dataSequence = new LinkedList<>();
            for (T data : copyPath.dataSequence) { //iterate through past Path's data and add that data to new path
                this.dataSequence.add(data);
            }
            this.dataSequence.add(extendBy.target.data); //append new data to Path
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
        if(start == null || end == null) { // check for null data pointers
            throw new NoSuchElementException("Either start or end data parameters were null!");
        }

        // check that the parameter vertices are in the graph
        if(!this.containsVertex(start) || !this.containsVertex(end)) {
            throw new NoSuchElementException("Either start or end vertices couldn't be found!");
        }

        // Boolean hashtable for keeping track of visited vertices
        Hashtable<Vertex, Boolean> visited = new Hashtable<Vertex, Boolean>();

        // Path hashtable for storing the path to every vertex (or data item) in the graph
        Hashtable<T, Path> paths = new Hashtable<T, Path>();
        // add starting vertex as 0-distance path
        Path startPath = new Path(vertices.get(start));
        paths.put(vertices.get(start).data, startPath);

        PriorityQueue<Path> pq = new PriorityQueue<Path>(); // create new priority queue
        pq.add(startPath); // add starting path

        Path current;
        Path newPath;
        while(!(pq.size() == 0)) { // while there are paths in the queue
            current = pq.remove(); // set current as head of pq
            visited.put(current.end, true); // mark current vertex as visited

            for (Edge e : current.end.edgesLeaving) { // for each successor
                Vertex successor = vertices.get(e.target.data);
                //ignore visited vertices
                if(visited.containsKey(successor) && visited.get(successor).booleanValue() == true) {
                    continue;
                }

                // now for each *unvisited* successor
                // if there is not a path to this successor, create one
                if(!paths.containsKey(successor.data)) {
                    newPath =  new Path(paths.get(current.end.data), e);
                    paths.put(successor.data, newPath);
                    pq.add(newPath); // insert successor into pq
                }
                // else check if the path can be shortened, and if so, update the path
                // more explicity, if the recorded path to the successor is greater than
                // the recorded path to current + the edge weight of current->successor
                else if(paths.get(successor.data).distance > paths.get(current.end.data).distance + this.getWeight(current.end.data, successor.data)) {
                    // update path
                    newPath = new Path(paths.get(current.end.data), e);
                    paths.replace(successor.data, newPath);
                    pq.add(newPath); // insert successor into pq
                }
            }
        }

        // check that the desired path exists
        if(!paths.containsKey(end)) {
            throw new NoSuchElementException("No path from start to end was found!");
        }

        // return the shortest path from start to end vertices
