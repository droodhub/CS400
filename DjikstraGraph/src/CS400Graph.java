...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
        //null checks before the method actually runs
        if(start == null || end == null) throw new NullPointerException("Cannot contain weighted edge adjacent to null data");
        Vertex startVertex = vertices.get(start);
        Vertex targetVertex = vertices.get(end);
        if(startVertex == null || targetVertex == null) throw new NoSuchElementException("Cannot find shortest path to/from nonexistent node");

    PriorityQueue<Path> frontier = new PriorityQueue<Path>(); //contains all nodes that can possibly be visited at the time
    ArrayList alreadyVisited = new ArrayList(); //contains any nodes already visited
    frontier.add(new Path(startVertex)); //initializes the frontier with the start vertex
    while(!frontier.isEmpty()){
        alreadyVisited.add(frontier.peek().end);
        Path current = frontier.poll();
        if(current.end == targetVertex)
            return current;
        for (Edge e : current.end.edgesLeaving){
            if(!alreadyVisited.contains(e.target))
                frontier.add(new Path(current, e));
        }
    }
    throw new NoSuchElementException("No path from start vertex to destination.");
    }
